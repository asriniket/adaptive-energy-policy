...
class EqMStateActionStateTrainer(Trainer):
    def __init__(
        self,
        network,
        dataset,
    ...
    def compute_eqm_loss(self, obs, action, next_obs):
        B = next_obs.shape[0]

        gamma = torch.rand(B, 1, device=self.device)
        noise = torch.randn_like(next_obs)

        interpolated = gamma * next_obs + (1 - gamma) * noise
        c_gamma = self.c_decay(gamma)

        target = (noise - next_obs) * c_gamma
        predicted = -self.network.velocity(obs, action, interpolated)
        return F.mse_loss(predicted, target)
    ...
    def train(self, iterations):
        ...
        pbar = tqdm(range(iterations), desc="Training EqM (s,a,s')")
        for _ in pbar:
            try:
                batch = next(data_iter)
            except StopIteration:
            ...
            self.optimizer.zero_grad()
            loss = self.compute_eqm_loss(obs, action, next_obs)
            loss.backward()
            self.optimizer.step()
            self.update_ema()
    ...
    def sample(self, obs, action):
        ...
        B = obs.shape[0]
        next_obs = torch.randn(B, self.obs_dim, device=self.device)

        for _ in range(self.num_sampling_steps):
            grad = -self.ema_network.velocity(obs, action, next_obs)
            next_obs = next_obs - self.sampling_step_size * grad
